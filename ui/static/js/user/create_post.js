...
async function loadCSRFTokenFromSession() {
    try {
        ...
        const csrfCookie = document.cookie.split('; ').find(row => row.startsWith('csrf_token_frontend='));
        if (csrfCookie) {
...
createBtn.addEventListener("click", async (e) => {
    ...
    try {
        ...
        if (!resp.ok) {
            const errorText = await resp.text();
            throw new Error(`Failed to fetch categories: ${resp.status} - ${errorText}`);
        }
        ...
        categoryCheckboxesContainer.innerHTML = "<p class='error'>Failed to load categories</p>";
    }
...
function clearModalInputs() {
    ...
    updateTitleCount();
    updateBodyCount();
    categoryCheckboxesContainer.querySelectorAll("input[type=checkbox]").forEach(cb => cb.checked = false);
      imageInput.value = "";
    imageError.textContent = "";
}
...
submitPostBtn.addEventListener("click", async () => {
    ...
    const categoryIDs = Array.from(categoryCheckboxesContainer.querySelectorAll("input[type=checkbox]:checked"))
